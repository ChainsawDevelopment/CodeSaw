...
const Home = () => (
    <span>
        <h1>
            <span className="logo" dangerouslySetInnerHTML={{ __html: logo }}></span>CodeSaw
        </h1>
...
const Review = withRouter((props: RouteComponentProps<{ projectId: string; id: string; fileId: FileId }>) => {
    ...
    return <ReviewPage reviewId={reviewId} history={props.history} />;
});
...
const Layout = (props: StateProps) => (
    <Dimmer.Dimmable as="div">
        ...
        <Menu inverted>
            <Container fluid>
                <Menu.Item as={(props) => <Link to="/" {...props} />}>Home</Menu.Item>
                <Menu.Item as={(props) => <Link to="/admin" {...props} />}>Administration</Menu.Item>
