...
const expectUnreviewedFileMarkedAsReviewed = (baseReviewInfo: ReviewInfo, reviewedAt: number): Mocha.Func => {
    return () => {
        const reviewInfo = B.applyBuilders(
            baseReviewInfo,
        ...
        const unpublished: UnpublishedReview = {
            ...
            ...forHead(reviewInfo),
            unpublishedReviewedFiles: [{ revision: RevisionId.makeSelected(reviewedAt), fileId: FileIds.file4 }],
        };
        const fileIdMap = {
    ...
    return () => {
        const reviewInfo = B.applyBuilders(
            ...
            B.addFileToReview(
                FileIds.file4,
                'file4',
                RevisionId.makeSelected(reviewedAt),
                RevisionId.makeSelected(reviewedAt),
            ),
            B.defineFileMatrix(),
        );
        const unpublished: UnpublishedReview = {
            ...emptyUnpublishedReview,
            ...forHead(reviewInfo),
            unpublishedUnreviewedFiles: [{ revision: RevisionId.makeSelected(reviewedAt), fileId: FileIds.file4 }],
        };
        const fileIdMap = {
            [FileIds.file4]: 'file4/new.txt',
        };
...
const expectFileDiscussionForSavedRevision = (
    baseReviewInfo: ReviewInfo,
    revision: number,
    fileId: FileId,
    fileName: string,
): Mocha.Func => {
...
describe('Upgrade review', () => {
    describe('Two revisions, no provisional', () => {
        const base = B.buildReviewInfo(B.addRevision(1), B.addRevision(2));
    ...
    describe('Two revisions, provisional', () => {
        const base = B.buildReviewInfo(B.addRevision(1), B.addRevision(2), B.addProvisional('P_HEAD_1', 'P_BASE_1'));
        ...
        it('provisional file marked as reviewed at P', () => {
            ...
            const unpublished = {
                ...emptyUnpublishedReview,
                ...forHead(reviewInfo),
                unpublishedReviewedFiles: [{ revision: RevisionId.Provisional, fileId: FileIds.prov_file3 }],
            };
            const fileIdMap = {
        ...
        it('known file marked as reviewed at P', () => {
            ...
            const unpublished = {
                ...emptyUnpublishedReview,
                ...forHead(reviewInfo),
                unpublishedReviewedFiles: [{ revision: RevisionId.Provisional, fileId: FileIds.file4 }],
            };
            const fileIdMap = {
                [FileIds.file4]: 'file4/new.txt',
            };
        ...
        it('known file marked as reviewed at P, file vanished', () => {
            ...
            const unpublished = {
                ...emptyUnpublishedReview,
                ...forHead(reviewInfo),
                unpublishedReviewedFiles: [{ revision: RevisionId.Provisional, fileId: FileIds.prov_file4 }],
            };
            const fileIdMap = {
        ...
        it('file discussion at P', () => {
            const reviewInfo = B.applyBuilders(
                base,
                B.addFileToReview(FileIds.prov_file3, 'file_prov', RevisionId.Base, RevisionId.Provisional),
                B.defineFileMatrix(),
            );
            const unpublished: UnpublishedReview = {
                ...emptyUnpublishedReview,
    ...
    describe('Three revisions, provisional is now saved', () => {
        const base = B.buildReviewInfo(B.addRevision(1), B.addRevision(2), B.addRevision(3));
        ...
        it('known file marked as reviewed at P (now R3)', () => {
            ...
            expect(upgradeReview(reviewInfo, unpublished, fileIdMap)).to.deep.equal({
                ...
                headCommit: 'REV_3_HEAD',
                baseCommit: 'REV_3_BASE',
                unpublishedReviewedFiles: [{ revision: RevisionId.makeSelected(3), fileId: FileIds.file4 }],
            });
        });
        ...
        it('known file marked as reviewed at P (now R2)', () => {
            ...
            expect(upgradeReview(reviewInfo, unpublished, fileIdMap)).to.deep.equal({
                ...
                headCommit: 'REV_3_HEAD',
                baseCommit: 'REV_3_BASE',
                unpublishedReviewedFiles: [{ revision: RevisionId.makeSelected(2), fileId: FileIds.file4 }],
            });
        });
    ...
    it('haha', () => {
        const reviewInfo = B.buildReviewInfo(
            B.addRevision(1),
        ...
        const unpublished: UnpublishedReview = {
            ...
            unpublishedReviewedFiles: [{ revision: RevisionId.makeSelected(1), fileId: FileIds.file4 }],
        };
        const fileIdMap = {
            [FileIds.file4]: 'file4/new.txt',
        };
