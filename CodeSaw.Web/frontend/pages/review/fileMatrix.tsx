...
const FileDiscussionSummary = (props: { discussions: FileDiscussion[] }): JSX.Element => {
    ...
    const label = (
        <Label as="span">
            {unresolved.length}/{discussions.length}
        </Label>
    );
    ...
    if (unresolved.length > 0) {
        content = [
            ...
            <ul key="unresolved-list">
                {unresolved.map((d) => (
                    <li key={d.id}>
                        line {d.lineNumber} from {d.comment.author.name}
                    </li>
                ))}
            </ul>,
        ];
    ...
    return <Popup trigger={label}>{content}</Popup>;
};
...
const MatrixCell = (props: {
    ...
    if (props.discussions.length > 0) {
        discussions = <FileDiscussionSummary discussions={props.discussions} />;
    }
...
const MatrixRow = (props: {
    ...
    for (const r of revisions) {
        ...
        const revisionDiscussions = props.discussions.filter(
            (f) => RevisionId.equal(r.revision, f.revision) && f.fileId == props.file.fileId,
        );
    ...
    return (
        <Table.Row>
            <Table.Cell key="file" className="file">
                <FileLink reviewId={reviewId} fileId={props.file.fileId}>
                    {file.newPath}
                </FileLink>{' '}
...
const fileMatrixComponent = (props: Props): JSX.Element => {
    ...
    const headers = props.revisions.map((i) => (
        <Table.HeaderCell key={i} className="revision">
            {i}
        </Table.HeaderCell>
    ));

    headers.unshift(
        <Table.HeaderCell key={'base'} className="revision">
            &perp;
        </Table.HeaderCell>,
    );
    ...
    if (props.hasProvisional) {
        headers.push(
            <Table.HeaderCell key={'provisional'} className="revision" style={{ fontStyle: 'italic' }}>
                P
            </Table.HeaderCell>,
        );
    }
    ...
    const rows = [];
    for (const entry of props.matrix) {
        ...
        rows.push(
            <MatrixRow
                key={entry.file.newPath}
                file={entry}
                review={review}
                reviewId={props.reviewId}
                discussions={props.fileDiscussions}
            />,
        );
    }
                    ...
                    <Table.Row>
                        <Table.Cell textAlign="left">
                            <Checkbox
                                ...
                                label="Hide reviewed"
                                onChange={(_, e) =>
                                    setShowOptions((showOptions) => ({ ...showOptions, hideReviewed: e.checked }))
                                }
                            />
                            <br />
                            <Checkbox
                                toggle
                                label="Hide without comments"
                                onChange={(_, e) =>
                                    setShowOptions((showOptions) => ({
                                        ...showOptions,
                                        hideWithoutComments: e.checked,
                                    }))
                                }
                            />
                        </Table.Cell>
