...
import { loadReviewInfo } from './state';
import { ReviewInfo, ReviewId, FileId } from '../../api/reviewer';
...
const LinkMenuItem = (props: { match: string; params?: any; children?: React.ReactNode }) => {
    ...
    return (
        <Menu.Item as={Link} to={to} active={match != null}>
            {props.children}
        </Menu.Item>
    );
};
...
const RoutedFile = (props: { reviewId: ReviewId }): JSX.Element => {
    ...
    return <File reviewId={props.reviewId} fileId={match.params.fileId} history={history} />;
};
...
const mapDispatchToProps = (dispatch: Dispatch): DispatchProps => ({
    loadReviewInfo: (reviewId: ReviewId, fileToPreload?: string) =>
        dispatch(loadReviewInfo({ reviewId, fileToPreload })),
});
