...
const NodeFileLink = (props: FileNodeProps) => {
    if (props.selectedFile != props.file.value.fileId) {
        return (
            <FileLink reviewId={props.reviewId} fileId={props.file.value.fileId}>
                {props.file.name}
            </FileLink>
        );
    } else {
...
const commentIcon = (
    ...
    return <Popup key={key} trigger={label} content={popupLabel} position="bottom center" size="tiny" />;
};
...
const FileNode = (props: FileNodeProps): JSX.Element => {
    ...
    return (
        <List.Item className={classNames({ file: true, selected })}>
...
const ReviewFilesTree = (props: StateProps): JSX.Element => {
    ...
    return (
        <List className="review-files-tree">
            {folders}
            {files}
        </List>
    );
...
const buildFileStats = (review: ReviewState) => {
    ...
    for (const item of review.unpublishedFileDiscussions) {
    ...
    for (const item of review.currentReview.fileDiscussions) {
        if (!result[item.fileId]) {
            result[item.fileId] = { ...emptyFileStats };
        }
