...
const ToggleSidebar = (props: { visible: boolean; set(visible: boolean): void }) => {
    if (props.visible) {
        return (
            <Menu.Item key="toggle-sidebar" fitted>
                <Popup
                    trigger={
                        <Icon
...
const DiffHeader = (props: Props): JSX.Element => {
    ...
    if (selectedFile) {
        ...
        menuItems.push(
            <RIMenu.ToggleReviewed
                key="review-mark"
                isReviewed={props.selectedFile.isReviewed}
                onChange={changeFileReviewState}
            />,
        );

        menuItems.push(
            <RIMenu.RefreshDiff key="refresh-diff" onRefresh={() => props.onSelectFileForView(selectedFile.fileId)} />,
        );

        menuItems.push(
            <RIMenu.FileNavigation
                key="file-navigation"
        ...
        if ((props.vsCodeWorkspace || '').length > 0) {
            menuItems.push(
                <RIMenu.OpenVSCode
                    key="vscode-diff"
                    workspace={props.vsCodeWorkspace}
        ...
        menuItems.push(
            <Menu.Item fitted key="revision-select">
