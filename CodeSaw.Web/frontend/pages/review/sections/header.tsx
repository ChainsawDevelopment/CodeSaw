...
const VSCodeWorkspaceEditor = (props: { path: string; save: (newPath: string) => void }): JSX.Element => {
    ...
    return (
        <Input
            fluid
            action={<Button primary icon="save" onClick={() => props.save(value)} />}
    ...
    return (
        <Grid.Row>
            <Grid.Column className={'header'}>
                <Grid.Row>
                    <h1>
                        Review {props.currentReview.title} <ExternalLink url={props.currentReview.webUrl} />
                    </h1>
                    <h3>
                            ...
                            <Dropdown floating inline icon="setting">
                                <Dropdown.Menu>
                                    <Dropdown.Item onClick={() => setShowVsCodeWorkspaceEditor(true)}>
                                        <img src={vscodeLogo} /> Set VS Code workspace for{' '}
                                        {props.currentReview.projectPath}
                                    </Dropdown.Item>
                                </Dropdown.Menu>
                            </Dropdown>
                    ...
                    {showVsCodeWorkspaceEditor ? (
                        <VSCodeWorkspaceEditor
                            path={props.vsCodeWorkspace}
                            save={(vsCodeWorkspace: string) => {
