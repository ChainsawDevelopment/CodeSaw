...
const describeFileOperations = (treeEntry: FileToReview): JSX.Element => {
    ...
    if (changeType == 'renamed') {
        return (
            <div key="renamed" className="file-operations">
                File renamed: <pre>{path.oldPath}</pre> &rarr; <pre>{path.newPath}</pre>
            </div>
        );
    } else if (changeType == 'deleted') {
        return (
            <div key="deleted" className="file-operations">
                File deleted: <pre>{path.oldPath}</pre>
            </div>
        );
    } else if (changeType == 'created') {
        return (
            <div key="created" className="file-operations">
                File created: <pre>{path.newPath}</pre>
            </div>
        );
    } else {
        return null;
        ...
        <Message className="file-summary">
            <Message.Content>
                <div>
                    ...
                    <strong>
                        {' '}
                        <FriendlyRevisionId revision={props.file.fileToReview.previous} />{' '}
                    </strong>
                    to{' '}
                    <strong>
                        {' '}
                        <FriendlyRevisionId revision={props.file.fileToReview.current} />{' '}
                    </strong>
                </div>
                {items}
            </Message.Content>
